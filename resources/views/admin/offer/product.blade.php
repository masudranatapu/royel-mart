                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-12">
                                    <h2>
                                        Offer Products <small class="badge bg-success text-white">{{ $qs_products->count() }}</small>
    ...
    <script type="text/javascript">
        function deleteData(id) {
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ route('admin.offer-product-delete') }}",
                        type:"POST",
                        data:{
                            _token: '{{csrf_token()}}',
                            offer_pro_id: id,
        ...
        function updateQuickSaleProduct(){
            $('#update-offer-product-form').submit();
        }
    </script>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#select_product_id').on('change', function(){
                ...
                $.ajax({
                    url: "{{ route('admin.add-product-to-offer') }}",
                    type:"POST",
                    data:{
                        _token: '{{csrf_token()}}',
