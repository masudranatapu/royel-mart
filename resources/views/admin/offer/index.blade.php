                ...
                <div class="page-body">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6">
                                    <h2>Offer <small class="badge bg-success text-white">{{ $sales->count() }}</small></h2>
                                </div>
                                        ...
                                        <div class="modal-dialog modal-lg" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form class="text-left" action="{{ route('admin.offer.store') }}" method="POST" enctype="multipart/form-data">
                                                        @csrf
                                        ...
                                        @foreach($sales as $key => $sale)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <a href="{{ route('admin.offer-product', $sale->id) }}" title="Set products for this sale" class="btn btn-info">
                                                        <i class="fa fa-plus"></i> Set Products
                                                    </a>
                                                </td>
                                                <td class="text-center">
                                                    <a href="{{ route('admin.offer-activity', $sale->id) }}" title="@if($sale->status == 1) Click for deactive @else  Click for active @endif" class="btn @if($sale->status == 1) btn-success @else  btn-danger @endif">
                                                        @if($sale->status == 1)
                                                            Active
                                                        @else
                                                            Inactive
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <form id="delete-form-{{ $sale->id }}" action="{{ route('admin.offer.destroy', $sale->id) }}" method="POST" style="display: none;">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                </td>
                                                <div class="modal fade" id="large-Modal-edit{{$key}}" tabindex="-1" role="dialog">
                                                    <div class="modal-dialog modal-lg" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form class="text-left" action="{{ route('admin.offer.update', $sale->id) }}" method="POST" enctype="multipart/form-data">
                                                                    @csrf
                                                                    @method('PUT')
