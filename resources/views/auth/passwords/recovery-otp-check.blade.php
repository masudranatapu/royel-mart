    ...
    <section class="phone-verification-section">
        <div class="container">
            ...
            <div class="verification-body">
                <form method="POST" action="{{ route('recovery-otp-resend') }}">
                    @csrf
                    <input class="form-control" placeholder="Enter Your Phone Number" value="{{ $recoveryPhone }}" readonly type="number" name="phone">
                    <button class="verify-btn">Resend PIN</button>
                </form>
                <form method="POST" action="{{ route('recovery-otp-check') }}" class="send-pin">
                    @csrf
                    <div class="mini-toast">
                        <span>We've sent a 5-digit one time PIN in your phone# {{ $recoveryPhone }}, Please type PIN</span>
